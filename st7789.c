...
void spi_master_init(TFT_t *dev, spi_host_device_t spi_host, int16_t GPIO_MOSI,
...
// color:color
void lcdDrawRegularPolygon(TFT_t *dev, uint16_t xc, uint16_t yc, uint16_t n,
...
// code:character code
// color:color
int lcdDrawCode(TFT_t *dev, FontxFile *fx, uint16_t x, uint16_t y, uint8_t code,
                uint16_t color) {
  if (_DEBUG_) printf("code=%x x=%d y=%d\n", code, x, y);
  if (dev->_font_direction == 0) x = lcdDrawChar(dev, fx, x, y, code, color);
  if (dev->_font_direction == 1) y = lcdDrawChar(dev, fx, x, y, code, color);
  if (dev->_font_direction == 2) x = lcdDrawChar(dev, fx, x, y, code, color);
  if (dev->_font_direction == 3) y = lcdDrawChar(dev, fx, x, y, code, color);
  if (dev->_font_direction == 0) return x;
  if (dev->_font_direction == 2) return x;
  if (dev->_font_direction == 1) return y;
  if (dev->_font_direction == 3) return y;
...
void lcdWrapArround(TFT_t *dev, SCROLL_TYPE_t scroll, int start, int end) {
  ...
  if (scroll == SCROLL_RIGHT) {
    ...
    for (int i = start; i < end; i++) {
      ...
      memcpy((char *)&dev->_frame_buffer[index1 + 1], (char *)&wk[0],
             (_width - 1) * 2);
    }
    ...
    for (int i = start; i < end; i++) {
      ...
      memcpy((char *)&dev->_frame_buffer[index1], (char *)&wk[1],
             (_width - 1) * 2);
    }
